...
class session():
    ...
    def getpars(self,fname=None):
        ...
        a  = re.search(r'\[\[Variables\]\][\n\s\W\w]*?\[\[',A)
        a1 = a.group()    
        gg = re.findall(r'\s*(\S*):\s*(\S*)\s(\S*)',a1)
    ...
    def save(self,outfname=None,outfig=True):
        ...
        if outfig == True:
            ...
            pyp.close()
            pyp.ion()
    # 25 Ago 2025 - Giuliano         
    def derived_parameters(self, fitresult=True):
        """ Devuelve parámetros derivados con unidades y errores. """
        import uncertainties.unumpy as unumpy
        ...
        def safe(x): return 0 if x is None else x

        emus  = list(map(safe, emus))
        esigs = list(map(safe, esigs))
        eNs   = list(map(safe, eNs))

        mus  = np.array([un.ufloat(mus[i], emus[i]) for i in range(self.ndist)])
        sigs = np.array([un.ufloat(sigs[i], esigs[i]) for i in range(self.ndist)])
        Ns   = np.array([un.ufloat(Ns[i], eNs[i]) for i in range(self.ndist)])

        mums  = unumpy.exp(mus) * unumpy.exp(sigs**2 / 2.)  # <mu>_number
        mu2ms = unumpy.exp(2 * mus) * unumpy.exp(2 * sigs**2)  # <mu^2>_number

        mmuN  = sum(mums * Ns) / sum(Ns)      # <mu>_number
        mmu2N = sum(mu2ms * Ns) / sum(Ns)     # <mu^2>_number
        SD    = unumpy.sqrt(mmu2N - mmuN**2)  # stddev
        mmumu = mmuN * ((SD / mmuN)**2 + 1)   # <mu>_mu
        N     = sum(Ns)
        Ms    = muB * sum(Ns * mums)
        ...
        if self.mass is not None and not self.divbymass:
            result["m_s/mass (units)"] = Ms / self.mass

        # Chi2
        Yteo = fitfunc(params, self.X, ndist=self.ndist)
        ssqua = sum((self.Y - Yteo)**2)
        result["sum squares"] = ssqua
